...
void writeLogString(std::string text, std::string path, std::string name)
{
    ...
    if (path == "")
    {
        LogFullPath = string_format("%s %04d-%02d-%02d.log", name, SystemTime.wYear, SystemTime.wMonth, SystemTime.wDay);
    }
    else
    {
        LogFullPath = string_format("%s\\%s %04d-%02d-%02d.log", path, name, SystemTime.wYear, SystemTime.wMonth, SystemTime.wDay);
    }
